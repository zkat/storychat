...
if(window.history && history.pushState) {
  ...
  // To detect when someone calls push/replaceState, we need to wrap each
  // method.
  can.each(["pushState","replaceState"],function(method) {
    var orig = history[method];
    history[method] = function() {
      console.log("Calling ", method, " with ", arguments);
  ...
  can.extend(can.route, {
    ...
    _setup: function() {
      ...
      can.$("body").on("click", "a", function(e) {
        if(!e.isDefaultPrevented()) {
          ...
          // HTML5 pushstate requires host to be the same. Don't prevent default
          // for other hosts.
     ...
     *
     * You may prefer to use replaceState instead of pushState in some
     * circumstances, in which case you can overwrite this method and handle the
     * change yourself.
     */
