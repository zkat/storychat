...
* active [0/6]
** TODO Session management                                    :sessions:auth:
   Set up a login/session management system with proper redirects.
** TODO Use redis for express session storage                 :sessions:auth:
   The default session storage system is neither persistent nor secure.
** TODO Secure auth for sockjs clients                         :sockets:auth:
