...
* backlog [0/30]
** TODO Connect/disconnect notifications for when people join/leave      :ui:
   Would be nice to know when someone's entered/left the chat.
...
** TODO Try a big blockquote-style quote thing for dialogue input        :ui:
** TODO Move/change the two dropdowns in chatInput                       :ui:
   For the type dropdown, it might be interesting to change it to radio buttons
   that are grouped pills.
   
   For the character input, it might be better to just make the character name
   in the text input area clickable (the <actor> elements), and let the user
   pick a character off that. Maybe even through a modal, so they can see their
   descriptions?
...
** TODO Entry removal support                                            :ui:
   ...
   that removal in their own chats. It might be significantly easier to do once
   the work needed for persistent chatlogs is complete.
** TODO Basic action 'parser'                                          :chat:
